#!/usr/bin/env bash
# Ejecuta complementaria all ficha para un mes/año (o rango vía variables de entorno).
#
...
for anio in "${ANIOS[@]}"; do
  ...
  for mes in "${MESES[@]}"; do
    ...
    echo "==============================="
    echo "Procesando periodo $periodo (ficha)"
    echo "==============================="
    ...
    while [[ $intento -le $MAX_REINTENTOS ]]; do
      echo "Ejecutando ficha intento $intento periodo $periodo"

      if "$PYTHON" "$SCRIPT_DIR/script.py" --periodo "$periodo"; then
        echo "OK ficha $periodo"
        success=1
        break
      else
    ...
    if [[ $success -eq 0 ]]; then
      echo "FALLO DEFINITIVO ficha $periodo"
    fi
  done
done

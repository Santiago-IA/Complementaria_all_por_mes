#!/usr/bin/env bash
# Ejecuta complementaria all aprendiz para un mes/año (o rango vía variables de entorno).
#
...
for anio in "${ANIOS[@]}"; do
  ...
  for mes in "${MESES[@]}"; do
    ...
    echo "==============================="
    echo "Procesando periodo $periodo (aprendiz)"
    echo "==============================="
    ...
    while [[ $intento -le $MAX_REINTENTOS ]]; do
      echo "Ejecutando aprendiz intento $intento periodo $periodo"

      if "$PYTHON" "$SCRIPT_DIR/script.py" --periodo "$periodo"; then
        echo "OK aprendiz $periodo"
        success=1
        break
      else
        echo "ERROR aprendiz $periodo"
        ((intento++)) || true
      fi
    done

    if [[ $success -eq 0 ]]; then
      echo "FALLO DEFINITIVO aprendiz $periodo"
    fi
  done
done
